import 'package:flutter/material.dart';
import 'package:lipsum/lipsum.dart' as lipsum;
import 'package:tda_academy/models/model_barrel.dart';
import 'package:tda_academy/screens/QuizExamScreen.dart';
import 'package:tda_academy/utils/SDUtils/SDColors.dart';
import 'package:tda_academy/utils/SDUtils/SDStyle.dart';

class QuizDetails extends StatefulWidget {
  final TestMaster testMaster;

  const QuizDetails({Key key, this.testMaster}) : super(key: key);
  @override
  _QuizDetailsState createState() => _QuizDetailsState();
}

class _QuizDetailsState extends State<QuizDetails> {
  var text;

  Color startColor = Colors.blue[400];
  Color endColor = Colors.blue[700];
  @override
  // ignore: must_call_super
  void initState() {
    text = lipsum.createText(numParagraphs: 1, numSentences: 5);
  }

  @override
  Widget build(BuildContext context) {
    Size size = MediaQuery.of(context).size;
    // changeStatusColor(this.startColor);
    return Scaffold(
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [
              this.startColor,
              this.endColor,
            ],
          ),
        ),
        height: size.height,
        width: size.width,
        padding: EdgeInsets.only(top: 25, left: 10),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Align(
              alignment: Alignment.topLeft,
              child: CloseButton(
                color: Colors.white,
              ),
            ),
            Container(
              margin: EdgeInsets.only(top: 50, left: 16),
              child: CircleAvatar(
                backgroundColor: Colors.white.withOpacity(0.3),
                child: Image.asset(
                  'assets/images/crops/119.png',
                  height: 60,
                  width: 60,
                ),
                radius: 40,
              ),
            ),
            Container(
              margin: EdgeInsets.only(top: 20),
              padding: EdgeInsets.only(left: 16),
              child: Text(
                '2020 Crash Quiz',
                style: boldTextStyle(
                  textColor: Colors.white,
                  letterSpacing: 0.2,
                ),
              ),
            ),
            Container(
              margin: EdgeInsets.only(top: 16, left: 16),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.center,
                children: <Widget>[
                  Text(
                    '40 questions',
                    style: secondaryTextStyle(
                      size: 14,
                      textColor: Colors.white.withOpacity(0.5),
                    ),
                  ),
                  SizedBox(
                    width: 10,
                  ),
                  Container(
                    decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        color: Colors.white.withOpacity(0.5)),
                    height: 4,
                    width: 4,
                  ),
                  SizedBox(
                    width: 10,
                  ),
                  Text(
                    '60 minutes',
                    textAlign: TextAlign.justify,
                    style: secondaryTextStyle(
                      size: 14,
                      textColor: Colors.white.withOpacity(0.5),
                    ),
                  )
                ],
              ),
            ),
            Expanded(
              child: Container(
                margin: EdgeInsets.only(top: 25),
                padding: EdgeInsets.only(left: 15, right: 15),
                child: Text(
                  text,
                  style: secondaryTextStyle(
                    size: 14,
                    textColor: Colors.white.withOpacity(0.5),
                  ),
                ),
              ),
            ),
            Container(
              alignment: Alignment.bottomCenter,
              margin: EdgeInsets.only(top: 25, bottom: 25),
              padding: EdgeInsets.only(left: 15, right: 15),
              child: InkWell(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => QuizExamScreen(),
                    ),
                  );
                },
                child: FittedBox(
                  child: Container(
                    margin: EdgeInsets.only(top: 20),
                    padding: EdgeInsets.fromLTRB(10, 4, 10, 4),
                    decoration: boxDecorations(
                      radius: 4,
                    ),
                    child: Center(
                      child: Text(
                        'Start Exam',
                        style:
                            boldTextStyle(size: 14, textColor: sdPrimaryColor),
                      ),
                    ),
                  ),
                ),
              ),
            )
          ],
        ),
      ),
    );
  }
}
